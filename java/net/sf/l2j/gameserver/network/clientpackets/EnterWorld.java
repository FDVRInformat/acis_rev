...
public class EnterWorld extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (Config.DONATESYSTEM && Config.SHOW_HTML)
		{
			Donate.showMainHtml(activeChar);
		}
	    ...
	    if (Config.ESPECIAL_VIP_LOGIN)
	    {
	    	if (activeChar.isVip())
	    	{
	    		Broadcast.announceToOnlinePlayers("Vip: "+activeChar.getName()+" has been logged in.");
		...
		if (Config.ENABLE_STARTUP)
		{
			...
			{
				NewbiesSystemManager.onEnterEquip(activeChar);
				NewbiesSystemManager.onEnterWepEquip(activeChar);
