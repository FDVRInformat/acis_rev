...
public abstract class ArenaTask
{
	...
	public static void SpawnEvent()
	{
		...
		Arena4x4.getInstance().clear();
		Arena9x9.getInstance().clear();
		
		spawnNpc1();
		
		Broadcast.gameAnnounceToOnlinePlayers("Tournament: Event Started");
		Broadcast.gameAnnounceToOnlinePlayers("Tournament: 2x2| 4x4 | 9x9");
		Broadcast.gameAnnounceToOnlinePlayers("Tournament: Duration: " + Config.TOURNAMENT_TIME + " minute(s)!");
		
		_aborted = false;
		_started = true;
		
		ThreadPool.schedule(Arena2x2.getInstance(), 5000);
		ThreadPool.schedule(Arena4x4.getInstance(), 5000);	
			...
			switch (seconds)
			{
				case 3600: // 1 hour left
					...
					if (_started)
					{
						...
						Broadcast.gameAnnounceToOnlinePlayers("Tournament: Reward: " + ItemTable.getInstance().getTemplate(Config.ARENA_REWARD_ID).getName());
