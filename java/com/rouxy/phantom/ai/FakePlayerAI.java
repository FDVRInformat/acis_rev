...
public abstract class FakePlayerAI
{
	...
	/*protected void refusalModeFake(Player player)
	{
		L2GamePacketHandler party;
		party.getClass().se
	...
	protected void handleDeath() 
	{
		...
		if(_fakePlayer.isDead()){	
			if(iterationsOnDeath >= toVillageIterationsOnDeath)
			{
				...
				if(Config.FAKE_PLAYER_RETURNS_TO_FARM_ZONE == true)
				{
	...
	protected void toVillageOnDeath() {
		...
		if (_fakePlayer.isDead() && !_fakePlayer.isInsideZone(ZoneId.FLAG) && !Config.FAKE_PLAYER_RETURNS_TO_FARM_ZONE)
			_fakePlayer.doRevive();
		//teleports fakeplayer to giran
	...
	protected void teleportBackFarmzone()
	{
		...
		_fakePlayer.getFakeAi().teleportToLocation(x, y, z, 20);
	}
